...
extension UIView
{
    fileprivate struct Constants
    {
        ...
        static let sliceSize: CGFloat = 01.0
        static let fps: TimeInterval = 60.0
        static let renderMargin: CGFloat = 0.0      //옆에 줄어들었다가 늘어남
    }
    ...
    fileprivate func genieTransition(duration: TimeInterval, edge: RectEdge, destinationRect destRect: CGRect, reverse: Bool, completion: (() -> ())?)
    {
        ...
        for i in 0 ..< Int(totalIter) {
            ...
            let curveP = self.progressOfSegmentWithinTotalProgress(a: Constants.curvesAnimationStart, b: Constants.curvesAnimationEnd, t: t)
            
            first.b[pAxis] = self.easeInOutInterpolate(t: curveP, a: bStartPoints.a[pAxis], b: bEndPoints.a[pAxis])
            second.b[pAxis] = self.easeInOutInterpolate(t: curveP, a: bStartPoints.b[pAxis], b: bEndPoints.b[pAxis])
            
            let slideP = self.progressOfSegmentWithinTotalProgress(a: Constants.slideAnimationStart, b: Constants.curvesAnimationEnd, t: t)
            
            let trs  = self.transformations(for: slices, edge: edge, startPosition: self.easeInOutInterpolate(t: slideP, a: first.a[axis], b: first.b[axis]), totalSize: totalSize, firstBezier: first, secondBezier: second, finalRectDepth: endRectDepth)
